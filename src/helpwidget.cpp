    ...
    : QWidget(parent)
{
    ...
    QSettings settings;
    this->resize(settings.value("HelpWidget/helpWindowSize", QSize(580, 700)).toSize());
    ...
    helpText.append(
        ...
        + "</li>"
          "<li>"
        + tr("Your operating system must be installed most probably from CD ROM. To do so, open "
             "the \"CD ROM\"-section.")
        + "</li>"
          "<li>"
    ...
    helpText.append(
        ...
        "<li>"
        "<strong>"
        + tr("New Machine: ") + "</strong>" + tr("Click here to create a new virtual machine.")
        + "</li>"
          "<li>"
          "<strong>"
        + tr("Open: ") + "</strong>" + tr("Click here to open an existing QtEmu machine (.qte).")
        + "</li>"
          "<li>"
          "<strong>"
        + tr("Power buttons (Start, Stop, Restart): ") + "</strong>"
        + tr("These buttons do the same as the buttons inside the machine configuration. The are "
    ...
    helpText.append("<ul>"
                    "<li>"
                    "<strong>"
                    + tr("\"MyMachines\"-Path: ") + "</strong>"
                    + tr("You can change the default \"MyMachines\"-path, which is in the folder "
                         "\"MyMachies\" in your private user / home folder. All new virtual "
                         "machines will be saved by default into the \"MyMachines\" folder.")
                    + "</li>"
                      "<li>"
                      "<strong>"
                    + tr("Tabbar position ") + "</strong>"
                    + tr("Here you can change the position of the tabbar (default: left).")
                    + "</li>"
                      "<li>"
                      "<strong>"
                    + tr("Language: ") + "</strong>"
                    + tr("The language is by default your system language if available. Feel free "
                         "to translate QtEmu into your language. Just contact the author.")
                    + "</li>"
                      "</ul>");
    helpText.append("<br />");
    ...
    helpText.append(
        ...
        "<li>"
        "<strong>"
        + tr("Snapshot Mode: ") + "</strong>"
        + tr("If you would like to test something in your virtual machine without have these "
             "changes on your next restart of your virtual system, activate \"Snapshot Mode\".")
        + "</li>"
          "<li>"
          "<strong>"
        + tr("Notes: ") + "</strong>"
        + tr("You are able to write down some note about the current virtual machine. Possible "
             "notes are for example applications which are installed.")
        + "</li>"
          "</ul>");
    helpText.append("<br />");
